...
- **Third-Party Codex Harness Keeps the Sol Prompt Without Disabling Workspace Tools**: Proxy-Chat catalog entries still inherit GPT-5.6 Sol's base instructions, model messages, reasoning menu, and supported tool metadata, but no longer copy the OpenAI-native `tool_mode=code_mode_only` and `use_responses_lite=true` transport flags. Those flags made Codex omit the classic tool definitions entirely on a Chat Completions route, leaving models to print fake `<tool_call>` text and silently finish without touching the workspace. Explicit first-turn workspace actions also map `tool_choice=auto` to `required` for that one turn, preventing models such as GLM-5.2 from ending after an "I'll inspect the files" promise; greetings, ordinary questions, explicit tool choices, and tool-result continuation turns remain automatic. An opt-in ignored live-proxy test host supports replaying the repository's public HumanEval/MBPP suites through the exact in-tree bridge.
- **Official Codex Compaction Crosses to Third Party Only for Explicit Account Quota Exhaustion**: Official compact requests now retry the same official route up to three times for transient 429, 5xx, timeout, connection-reset, and provider-health failures. Authentication errors, malformed requests, format/content errors, and ordinary transient failures never cross realms; only an explicit official hard-quota/account-limit response may use the configured third-party summarizer, logged as `Official compaction quota exhausted; using configured third-party summarizer`.

- **Third-Party Codex Harness Uses GPT-5.6 Sol Instead of GPT-5.5**: ProxyChat model-catalog entries now clone the exact `gpt-5.6-sol` base instructions, model messages, tool mode, multi-agent version, apply-patch/shell/web-search declarations, and supported-tool metadata from Codex's current catalog, falling back to GPT-5.5 only for older Codex installations. A bundled GPT-5.6 Sol template keeps first-run behavior consistent on computers without a populated model cache. Responses-to-Chat history also preserves prior hosted `web_search_call` actions as read-only assistant context instead of silently dropping them.

- **Codex Compaction Survives Text-Only Models and NVIDIA Worker Saturation**: LiteLLM's HTTP 404 "no image endpoint" response now triggers the existing media fallback, while transient NVIDIA worker/deployment capacity errors are retried three times (1s/2s/4s) without incorrectly quota-latching the provider.

- **Third-Party Codex Reasoning-Only Stops Are Rejected**: Chat Completions bridges no longer translate an upstream `finish_reason=stop` containing only hidden reasoning into a successful Codex completion. The bridge now emits an explicit `empty_completion` failure unless the model produced a user-visible answer or a valid tool call, preventing Codex Desktop from silently marking an unfinished task complete after a tool result.

- **Third-Party Codex Models Do Not Stop After Merely Announcing Work**: Responses-to-Chat requests with tools now include an explicit continuity contract requiring the model to issue the corresponding tool call in the same response whenever it says it will inspect, edit, run, or test something. This prevents models such as Nemotron from returning a normal successful completion like “Now I'll create the files” and silently ending the task without performing the announced action.
- **Official Responses WebSocket Honors the Selected CC Switch Account**: Native official-model WebSocket relay now replaces the Codex app's Authorization and ChatGPT account headers when `OpenAI Official` is bound to a managed/default CC Switch account. Previously HTTP official requests used the selected account while WebSocket requests silently retained the account signed into Codex Desktop, so usage could be charged to a different subscription and the relay left no ordinary provider request-log row.
- **Bridge Compaction Actually Replaces the Superseded Context Prefix**: Materializing a Codex compaction for GLM and other Chat/Anthropic bridges now discards the raw history before the checkpoint and keeps only the durable handoff summary plus the post-compaction suffix. The previous behavior sent both the full old transcript and its summary, defeating compaction, overflowing third-party context windows, and causing apparently random loss of the current task after automatic context reduction.
- **Auto Review Recognizes Real Codex Guardian Requests and Updates Counters**: Codex Desktop's approval Guardian keeps the thread's selected model id instead of sending the synthetic `codex-auto-review` model name. CC Switch now recognizes the stable Guardian instruction contract, so configured Auto Review routing is actually used and official attempts, fallbacks, successes, failures, and last-run details are persisted instead of remaining at zero.
- **GLM Chat Tool Continuations Recover From Illegal-Messages 400s**: Some LiteLLM-backed GLM routes reject an otherwise standard Chat Completions follow-up when the final message is a tool result, returning `The messages parameter is illegal`. For Responses-to-Chat bridges, CC Switch now recognizes only that exact HTTP 400 shape and retries once with a minimal user continuation after the tool output; unrelated 400s and providers that accept the standard tool tail remain untouched.
- **Third-Party Codex Thinking Matches Official Privacy Semantics**: Chat Completions reasoning deltas are still accumulated internally and attached to tool-call history, but are no longer streamed into the Codex UI as raw chain-of-thought text. The Responses bridge emits the reasoning lifecycle with an empty public summary, keeping the native “thinking” state visible while hiding provider-internal reasoning just like official Codex models.
- **Codex OAuth Recovers From Server-Side Token Invalidation**: CC Switch now treats a `401`/`403` from the ChatGPT quota endpoint as potentially recoverable even when the cached access token's JWT expiry is still in the future. It forces exactly one refresh-token exchange and retries the request once before asking the user to sign in again; a per-account lock plus rejected-token comparison prevents concurrent quota refreshes from rotating the same refresh token repeatedly. If OpenAI also rejects the stored refresh token, the UI keeps the upstream reason and correctly requires a new cc-switch login.
- **Official Codex WebSockets Preserve Mobile Presence and Remote Compaction**: Official model sockets are now relayed end-to-end to ChatGPT with the caller's OAuth, account, and feature headers instead of being terminated locally and translated into HTTP/SSE. This keeps the desktop's cloud presence visible to paired phones and preserves the native remote-compaction v2 event contract, preventing paired devices from reporting the computer offline and avoiding `Connection reset without closing handshake` / `expected exactly one compaction output item, got 0` failures. Managed third-party `ccs-*` models continue to use the local WebSocket-to-HTTP bridge so Chat Completions and Anthropic upstream conversion still work.
- **App Integration Tests Clean Up Tauri State Reliably**: The App integration harness now mocks the profile bootstrap and window APIs, clears mocked Tauri event listeners after every test, and gives the multi-provider flow an explicit integration-test timeout. The suite no longer spends its timeout resolving an unhandled `tauri.local/list_profiles` request or leaves duplicate React/Radix DOM behind for the next test.
- **Codex Coexistence Menu Mirrors Desktop and Keeps Every Selected Provider Model**: The coexistence setup no longer falls back to the ChatGPT account `/models` response for its official section, because that endpoint can expose rollout or legacy GPT ids (for example GPT-5.2) that native Codex Desktop intentionally omits. The native `models_cache.json` is now the only official-menu source, while every selected third-party route is appended with an explicit `[Provider] Model` label so it cannot be mistaken for an official GPT entry. Proxy activation also repairs the inconsistent legacy state where a provider still has its selected `modelCatalog` but the separate `codex_model_routes` setting is empty; the configured provider id is now remembered explicitly and, for an unambiguous older installation with one configured third-party provider, recovered automatically before takeover. This fixes the official-only/stale in-memory menu that survived even though the selected GLM/DeepSeek models were still present in the provider database.
- **Official Proxy Accepts Native Compactions Created While Proxy Was Off**: A ChatGPT compaction token created directly by Codex while CC Switch was disabled has no entry in CC Switch's canonical journal. Re-enabling official routing previously rejected that valid opaque token locally with HTTP 422 before contacting OpenAI. Unknown native compactions now pass through unchanged only to the fixed `codex-official` backend; third-party and bridge targets still fail closed to avoid dropping or leaking context.
- **Codex Model Menu Configuration Preserves the Native ChatGPT Login**: Saving a third-party model menu previously reused the generic provider-update path before enabling coexistence routing. If that provider was marked current, the update copied its API key into live `auth.json`, switched Codex Desktop from the native ChatGPT session to API-key mode, and left the sidebar showing `openai` with unusable subscription models. Model-menu saves now persist the provider catalog directly to CC Switch's database and let the dedicated routing transaction manage only the proxy configuration; they never apply third-party credentials to live Codex auth. Failures roll back both the route settings and stored catalog, and clearing the menu removes the stored catalog without touching the native login.
- **Codex Model Injection No Longer Falls Into Windows Onboarding**: Current Codex rejects `[model_providers.openai]` because `openai` is a reserved built-in provider id. The invalid takeover config made every app-server config reload fail, which the Desktop UI misleadingly surfaced as an unfinished Windows setup whose retry button could never succeed. Official and third-party model routing now keeps `model_provider = "openai"` for a consistent conversation/project history bucket and uses Codex's supported top-level `openai_base_url` override instead, while automatically cleaning the legacy CC Switch loopback override table. Generated proxy catalogs still prefer HTTP, but the local `/responses` routes now also accept Codex's native WebSocket upgrade: official sockets relay to ChatGPT unchanged, while third-party `response.create` frames use the HTTP/SSE conversion pipeline. This eliminates the first-turn `405 Method Not Allowed` reconnect without changing provider selection or error reporting.
- **Disabling Codex Proxy Restores the Native Model Menu Without Touching Login State**: Every successful disable path, including the already-disabled/idempotent path used after an account switch, now removes only CC Switch's generated `cc-switch-model-catalog.json` reference and file. User-managed catalogs are preserved, native ChatGPT OAuth remains byte-for-byte owned by Codex, and a stale third-party-only menu can no longer survive a proxy-off restart.
- **Codex OAuth Tokens Cannot Be Redirected to Third-Party Providers**: Direct-mode history normalization no longer relabels an OAuth-only third-party profile as the built-in `openai` provider or copies its remote URL into `openai_base_url`. A third-party direct profile is normalized into the shared history bucket only when it carries its own scoped API key or bearer token, preventing a native ChatGPT JWT from being forwarded to gateways that expect an `sk-` key.
- **Native Compact Responses May Return Replacement History Without an Opaque Marker**: Successful official `/responses/compact` JSON and SSE responses are no longer rejected merely because their `output` has no `{type: "compaction"}` item. Opaque items are still journaled when present, while ordinary replacement history now passes through unchanged, fixing the local HTTP 422 raised after an otherwise successful official compaction.
- **Auto Review Counters Survive Proxy Restarts**: Official attempts, quota detections, fallback outcomes, and the last fallback detail are persisted in CC Switch settings and restored whenever the proxy server is recreated, so successful third-party review fallback no longer leaves the UI counter at its initial value.
- **Codex Proxy Shows Upstream Errors Verbatim Before Route Context**: Codex-facing proxy errors no longer replace provider wording with hard-coded diagnoses such as “quota exhausted” or “quota insufficient.” The upstream message, type, and code remain authoritative and appear first, while CC Switch appends provider/model/endpoint/status only as separate route context. This keeps transient `Too many requests` distinct from persistent limits and preserves Chinese provider messages such as `所剩配额不足` exactly for debugging; internal quota classification is still used only for retry/latch policy.
- **Codex Windows Sandbox Mode Is Device-Local and Survives Proxy Injection**: Codex common-config extraction now treats `[windows].sandbox` as device-local instead of copying it between providers and machines. Carrying `sandbox = "elevated"` to a PC that has not completed Codex's Windows runtime provisioning can force the app into policy-required onboarding and disable its limited-access fallback, while deleting an already-provisioned PC's local value during proxy takeover can re-trigger the same onboarding. New and legacy snippets therefore omit only this provisioning-dependent key when shared, but applying, removing, enabling, or disabling proxy injection always preserves the current machine's live value. Unrelated `[windows]` preferences are still portable, and the model-catalog path continues to leave Codex's onboarding JSON untouched and never fabricate a runtime-ready state.
